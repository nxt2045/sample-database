...
"""
original src code is from : https://github.com/aysusayin/BPlusTree
only change a little for functions in this project
"""
...
class BPlusTree:
    ...
    def range_search(self, notation, cmp_key):
        notation_func = {
            ">": lambda key: self.root.find_right(key, False),
            "<": lambda key: self.root.find_left(key, False),
            "=": self.get,
            "!=": lambda key: self.root.find_left(key, False) + self.root.find_right(key, False),
            "<=": lambda key: self.root.find_left(key, True),
            ">=": lambda key: self.root.find_right(key, True),
        }
        if notation not in notation_func.keys():
            raise Exception("Nonsupport notation: {}. Only '>' '<' '>=' '<=' '=' '!='are supported")
        return notation_func[notation.strip()](cmp_key)
